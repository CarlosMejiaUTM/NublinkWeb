...
const StatCard = ({ title, value, icon: Icon }: { title: string; value: string; icon: React.ElementType }) => (
  <Card className="shadow-sm" hoverEffect>
            ...
            <tbody>
              {filteredTickets.length === 0 ? (
                ...
                filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="border-t border-line-light hover:bg-secondary-light transition-colors">
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
        <StatCard title="Tickets Abiertos (Total)" value={ticketsAbiertos.toString()} icon={TicketIcon} />
        <StatCard title="Tickets Nuevos (Sin Asignar)" value={ticketsNuevos.toString()} icon={ClockIcon} />
        <StatCard title="Resueltos (Hoy)" value={"0"} icon={CheckCircleIcon} />
      </div>
